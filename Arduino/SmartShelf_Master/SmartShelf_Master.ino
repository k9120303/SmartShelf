  ...
  {   
    ...
    if (Console.available()) {
     ...
     Console.print("Now Send To Product ");
      ...
      if( 10000 > ElapsedTime & ElapsedTime >= 3000 ) {
        look = look+1;
        sprintf(buffer, "HEAD /SmartShelf/row.php?temp=%d?$temp3=%d HTTP/1.1", look, key2 );     // localhost下的絕對路徑
        client.println(buffer);   //將字串傳到網頁端
      }
      if( 60000 > ElapsedTime & ElapsedTime >= 10000) {
        think = think +1;
        sprintf(buffer, "HEAD /SmartShelf/row.php?temp2=%d?&temp3=%d HTTP/1.1", think, key2 );     // localhost下的絕對路徑
        client.println(buffer);   //將字串傳到網頁端
      }
      //Console.println(ElapsedTime);
      Console.print("Look  Value ");
