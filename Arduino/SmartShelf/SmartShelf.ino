...
void loop()
{
  ...
  {   
    ...
    if (key2>0){
      Console.print("Now Send To Product ");
      Console.println(key2);   // 就印出按鍵對應的字元
      unsigned long NextTime = millis();
      unsigned long ElapsedTime = NextTime - ThisTime;
      if( 10000 > ElapsedTime & ElapsedTime >= 3000 ) look = look+1;
      if( 60000 > ElapsedTime & ElapsedTime >= 10000) think = think +1;
      //Console.println(ElapsedTime);
      Console.print("Now Send Look  Value ");
      Console.println(look);    //稍微看看
      Console.print("Now Send Think Value ");
      Console.println(think);   //非常猶豫
      Console.println();
      sprintf(buffer, "HEAD /SmartShelf/row.php?temp=%d?temp2=%d&temp3=%s HTTP/1.1", look, think, key2);     // localhost下的絕對路徑
      client.println(buffer);   //將字串傳到網頁端
      client.println("Host: localhost");
      client.println("Connection: Close");
      client.println();
