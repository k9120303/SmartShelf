...
echo '<div class="w3-top">
  <ul class="w3-navbar w3-black w3-card-2 w3-left-align">
	  ...
	  $look = "INSERT INTO grid (look) VALUES ('".@$_GET["look"]."')";
	  $think = "INSERT INTO grid (think) VALUES ('".@$_GET["think"]."')";
	  mysql_query($look);
	  mysql_query($think);
	  ...
	  $value = "SELECT * FROM grid";
	  $result = $db->query($value);

	  if ($result->num_rows > 0) {
		// output data of each row
		while($grid = $result->fetch_assoc()) {
			echo '<tbody class="table-hover">';
			echo '<tr>';
			echo '<td class="text-left">'. @$grid["grid_num"].'</td>';
			echo '<td class="text-left">'. @$grid["name"].'</td>';
			echo '<td class="text-left">'. @$grid["look"].'</td>';
			echo '<td class="text-left">'. @$grid["think"].'</td>';
